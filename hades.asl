...
startup
{
    ...
    vars.ReadOffset = (Func<Process, IntPtr, int, int, IntPtr>)(
        (proc, ptr, offsetSize, remainingBytes) => {
            byte[] offsetBytes;
...
split
{
    ...
    if (
        (settings["splitOnBossKill"] && (vars.boss_killed || vars.exit_to_hades)) // Split on boss kill
        || (!(settings["splitOnBossKill"]) && entered_new_room && in_postboss_room_or_hades_fight) // Split on room transition
        || vars.has_beat_hades // Always split on Hades Kill
    ) {
